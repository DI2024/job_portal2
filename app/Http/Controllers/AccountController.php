...
class AccountController extends Controller
{
    ...
    public function updateProfilePic(Request $request)
    {
        ...
        if ($validator->passes()) {
            $image = $request->file('image'); // Use the file method to get the image
            $ext = $image->getClientOriginalExtension();
            $imageName = $id . '-' . time() . '.' . $ext;
            $image->move(public_path('/profile_pic'), $imageName); // Fix the directory path
            ...
            User::where('id', $id)->update(['image' => $imageName]); // Fix the typo in method name

            session()->flash('success', 'Profile picture updated successfully.'); // Fix the typo in flash message key
            return response()->json([
                'status' => true,
                'errors' => []
            ]);
    ...
    public function deleteJob(Request $request)
    {
        ...
        if ($job == null) {
            session()->flash('Error', 'Job deleted or not found.');
            return response()->json([
                'status' => false,
            ]);
        }
    ...
    public function myJobApplications(){
        $jobApplications = JobApplication::where('user_id',Auth::user()->id)->orderBy('created_at', 'DESC')->with(['job','job.jobType','job.applications'])->paginate(10);
